...
First, let’s say we are interested in determining whether the population average height of female students exceeds 64.5 inches, and we aim to statistically confirm this claim at a significance level of 0.1.
As mentioned, the population average (or mean) is denoted by the Greek letter $\mu$. Thus, we express our claim as the alternative hypothesis: $\mu > 64.5$. The null hypothesis, being the opposite, is $\mu \le 64.5$:
$$
...
## Sample Data
As the target parameter population mean is unknown, we need to collect a sample for inference. For this purpose, let's use the **Height** variable in our class survey data. 
```{webr-r}
head(Survey_Data)
# Obtain the index of Female students
index<-Survey_Data$Gender == "Female"
# Obtain the Height data of Female students
X<-Survey_Data$Height[index]
```

## Choice of Test
Now, let us explore our data vector $X$ and record necessary components.
```{webr-r}
# Sample Size
print(n<-length(X))
# Mean value
...
alpha<-0.1
```
Here, the sample size is large $n\ge 30$, so we can use the $z$ test statistic:
$$
...
In a z-test, the p-value is essentially one of the tail-probabilities from the standard normal distribution. Since this test is a **right-tailed** test, we calculate the right-tail probability. In **R**, the function $\texttt{pnorm()}$ provides us the left-tail probability, so
$$
...
```
Note that the sample size of new data set is small ($n<30$). In this case, we have to use the $t$-test. (Let us assume the population distribution is normal to satisfy the assumption.)
$$
...
For the p-value calculation, we now need to compute one of the tail probabilities of the t-distribution with the corresponding $n-1$ degrees of freedom. The t-table we use in class does not provide the exact probability, but the **R** function $\texttt{pt()}$ can be used for the precise computation. Since this is still right-tailed test:
$$
\text{p-value}=P[T>t^*]\stackrel{\textsf{R}}{=}\texttt{1-pt(t.star,n-1)}.
$$
```{webr-r}
# p-value
