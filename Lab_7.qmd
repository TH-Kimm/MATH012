...
Our first step is to examine the correlation and scatter plots. We'll create these for the mother’s height ($\txt{MO_Height}$) and father’s height ($\txt{FA_Height}$) variables, respectively.
...
Note that the coefficient of determination ($\mathbf{R}^2$) is 0.3926, and it represents the proportion of variation in a student's height explained by the mother's height through the regression analysis. Currently, the R-squared value is not particularly high, indicating that the model’s predictive power is fairly limited.
