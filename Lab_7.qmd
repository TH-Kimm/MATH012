...
The $\txt{subset()}$ function takes three arguments: the first is the dataset to subset, the second is the condition for selecting observations (rows in data table), and the third specifies the variables (columns in data table) to retain. For example,
...
In fact, the $\txt{lm()}$ function provides much more information when used with the $\txt{summary()}$ function. While most of these details are beyond the scope of this class, we can identify the coefficient of determination ($\mathbf{R}^2$) — which is 0.3926 in this case — from the "Multiple R-squared" value in the summary.
```{webr-r}
## Regression summary
summary(Reg1)
```
