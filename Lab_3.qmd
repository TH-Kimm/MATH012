...
<!-- -->
```
2.  The sample variance has the following formula: 
$$s^2=\frac{1}{n-1}\sum_{i=1}^n\left(x_i-\bar{x}\right)^2,$$ where $x$ is a numerical variable of length $n$. Calculate the variance of $\texttt{SL}$ without using $\texttt{var()}$ function. (You could use $\texttt{mean()}$, $\texttt{sum()}$, or $\texttt{length()}$ functions.) Check your results coincides with the value in 1-ii.
