...
In this lab, we will use a hypothetical population with a mean $\mu=70.77$ and a standard deviation $\sigma=3.5$ for this scenario. Note that, **for illustration purposes**, we have access to all measurement data in this example.
```{webr-r}
...
```
From the population, we can randomly draw a sample of size $n=50$ using the $\texttt{sample()} function. Since the population is large enough, we are able to assume the obtained data is independent and identically distributed. 
```{webr-r}
# Random sampling 
n<-50
...
hist(my.sample)
```
Note that each time you repeat the sampling, the shape of the histogram will vary due to sampling variability. In principle, every student in the class would obtain a similar, though not identical, histogram from their sample.
...
Let us construct the 90\% confidence interval for the population mean (i.e., population average height).
```{webr-r}
## Three components
xbar <- mean(my.sample)
cut_off <- 1.645
...
In practice, it’s impossible to know whether a computed CI actually covers the target parameter. However, with our hypothetical population, we can check this!
```{webr-r}
...
We have constructed the 90\% confidence interval, but what does "90\% confidence" mean? 
It means that 90% of intervals from different samples will contain the true target parameter $\mu$.
...
```{webr-r}
# Null vector for the sample means
xbars <- NULL
# Calculate 50 sample means
for(i in 1:50){
  re.sample <- sample(M.Height,n)
  xbars[i] <- mean(re.sample)
}
# Three sets of CIs: 90%, 95%, and 99%
L.Bounds1 <- xbars - cut_off*se;U.Bounds1 <- xbars + cut_off*se
L.Bounds2 <- xbars - 1.96*se;U.Bounds2 <- xbars + 1.96*se
L.Bounds3 <- xbars - 2.58*se;U.Bounds3 <- xbars + 2.58*se
# Plots
par(mfrow=c(1,3))
CI_Plot(L.Bounds1,U.Bounds1,mu,main="Fifty 90% CIs")
CI_Plot(L.Bounds2,U.Bounds2,mu,main="Fifty 95% CIs")
CI_Plot(L.Bounds3,U.Bounds3,mu,main="Fifty 99% CIs")
par(mfrow=c(1,1))
```
You can run the code a few times, as the exact coverage may not be 90\%, 95\%, or 99\% due to randomness. However, on average, you should observe that approximately "5" CIs, "2 or 3" CIs, and "1" CI will not contain the true parameter out of a total of 50 CIs, respectively.

# Interpretation of confidence interval

Returning to the real case, note that we only observe a **SINGLE** sample. Since the target parameter is **UNKNOWN**, we can never know for sure whether the computed confidence interval (CI) contains the true parameter. Once the CI is computed, it consists of a fixed pair of constants, and since we cannot assign a probability to these fixed values, we use the term "confidence" instead. This indirect expression reflects our earlier observation that, **if the CI were constructed repeatedly, it would cover the true parameter based on the confidence level**.
